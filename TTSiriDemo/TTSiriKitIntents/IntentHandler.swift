...
class IntentHandler: INExtension,INSendMessageIntentHandling,INSearchForMessagesIntentHandling,INStartWorkoutIntentHandling, INPauseWorkoutIntentHandling, INResumeWorkoutIntentHandling, INCancelWorkoutIntentHandling, INEndWorkoutIntentHandling{
    
    var searchForResponse: INSearchForMessagesIntentResponse!
    ...
    //MARK: --confirm   Confirm阶段是我们最后可以尝试修改intent事件中传递的数值的时候。要记住一点，完全精确的内容固然是最好的答案，但是过多的让Siri询问用户参数的详细信息也会导致用户的抵触。
